		...
		for (int32 s = -1; s < Dims[d]; ++s)
		{
			...
			for (int32 j = 0; j < MaskH; ++j)
			{
				for (int32 i = 0; i < MaskW; ++i)
				{
					...
					// A face is needed at this boundary when one side renders (non-Air)
					// and the other side does not occlude it (is Air or non-opaque).
					// This handles opaque/transparent pairs AND non-opaque/Air pairs (e.g. Water surface).
					if (bAOpaque && !bBOpaque)
					{
						// Opaque A, transparent B → face with +d normal, owned by block A
						Mask[i + j * MaskW] = static_cast<int32>(BlockA);
					}
					else if (!bAOpaque && bBOpaque)
					{
						// Transparent A, opaque B → face with −d normal, owned by block B
						Mask[i + j * MaskW] = -static_cast<int32>(BlockB);
					}
					else if (bAVisible && !bBVisible)
					{
						// Non-opaque solid A (e.g. Water) against Air B → face with +d normal
						Mask[i + j * MaskW] = static_cast<int32>(BlockA);
					}
					else if (!bAVisible && bBVisible)
					{
						// Air A against non-opaque solid B (e.g. Water) → face with −d normal
						Mask[i + j * MaskW] = -static_cast<int32>(BlockB);
					}
					else
					{
						// Both opaque, both Air, or both same non-opaque type → no face
						Mask[i + j * MaskW] = 0;
					}
				}
			}
